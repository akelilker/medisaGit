/* =========================================
   ...
   (function () {
    ...
    // — Şube Detay Görünümü —
    window.showBranchDetail = function showBranchDetail(id) {
      ...
      const html = `
        <div class="settings-detail-view">
          <div class="settings-detail-header">
            <button class="settings-back-btn" onclick="renderBranchList()">
          ...
          <div class="settings-detail-info">
            <div class="settings-detail-item">${escapeHtml(branch.name)}</div>
            <div class="settings-detail-item">${escapeHtml(branch.city || '')}</div>
          </div>
        </div>
      `;
    ...
    window.showUserDetail = function showUserDetail(id) {
      ...
      const vehicleCount = vehicles.filter(v => v.assignedUserId === id).length;
  
      const html = `
        <div class="settings-detail-view">
          <div class="settings-detail-header">
            <button class="settings-back-btn" onclick="renderUserList()">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <path d="M19 12H5M12 19l-7-7 7-7"/>
              </svg>
            </button>
            <h3 class="settings-detail-title">${escapeHtml(user.name || 'İsimsiz')}</h3>
          </div>
          
          <div class="settings-detail-info">
            <div class="settings-detail-item">${escapeHtml(branchName)}</div>
            <div class="settings-detail-item">${escapeHtml(roleLabel)}</div>
            ${user.phone ? `<div class="settings-detail-item">${escapeHtml(user.phone)}</div>` : ''}
            ${user.email ? `<div class="settings-detail-item">${escapeHtml(user.email)}</div>` : ''}
          </div>
        </div>
      `;
