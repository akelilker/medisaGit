    ...
    function renderStokDetailMenu() {
        ...
        return `
                <div class="stok-detail-menu-item ${isActive ? 'draggable' : ''}" 
                     ...
                     ondragstart="handleStokColumnDragStart(event, '${opt.key}')"
                     ondragover="handleStokColumnDragOver(event)"
                     ondrop="handleStokColumnDrop(event, '${opt.key}')"
                     ondragenter="handleStokColumnDragEnter(event)"
                     ondragleave="handleStokColumnDragLeave(event)"
                     ondragend="handleStokColumnDragEnd(event)">
    ...
    window.showKullaniciDetail = function(userId) {
        ...
        if (userEvents.length === 0) {
            ...
            userEvents.forEach(event => {
                ...
                if (event.type === 'kaza') {
                    eventTypeLabel = 'KAZA';
                    const surucu = event.surucu || '-';
                    const tutar = event.tutar ? formatNumber(event.tutar) + ' TL' : '-';
                    eventText = `Kullanıcı: ${escapeHtml(surucu)} | Hasar: ${escapeHtml(tutar)}`;
                } else if (event.type === 'bakim') {
                    eventTypeLabel = 'BAKIM';
                    const islemler = event.islemler || '-';
                    const tutar = event.tutar ? formatNumber(event.tutar) + ' TL' : '-';
